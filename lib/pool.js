...
function templateUpdate(repeating) {
    global.coinFuncs.getBlockTemplate(global.config.pool.address, function (rpcResponse) {
        if (rpcResponse) {
            if (typeof(rpcResponse.result) === 'undefined' && typeof(activeBlockTemplate) !== 'undefined'){
                console.error("Error in GBT.  Skipping update, staying with BT at height: " + activeBlockTemplate.height);
                console.error("Response from Monerod: " + rpcResponse);
